...
func (route *AdminControllers) AdminRoutes(group *gin.RouterGroup) {
	...
	// Admin routes
	group.POST("/users", route.Admin.PostV1CreateAdmin)
	group.PATCH("/users", route.Admin.PatchV1UpdateAdmin)
	group.DELETE("/users", route.Admin.DeleteV1DeleteAdmin)
	group.POST("/users/login", route.Admin.PostV1LoginAdmin)
