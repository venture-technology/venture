...
func (cr *ContractRepository) FindAllByRg(ctx context.Context, rg *string) (*entity.Contract, error) {
	sqlQuery := `
		SELECT
	...
	var contract entity.Contract
	err := cr.db.QueryRowContext(ctx, sqlQuery, rg).Scan(
		&contract.Record,
		&contract.StripeSubscription.Title,
		&contract.StripeSubscription.Description,
		&contract.StripeSubscription.ID,
...
func (cr *ContractRepository) Cancel(ctx context.Context, id uuid.UUID) error {
	sqlQueryUpdate := `UPDATE contracts SET status = 'canceled' WHERE id = $1`
	_, err := cr.db.ExecContext(ctx, sqlQueryUpdate, id)
	return err
}

func (cr *ContractRepository) Expired(ctx context.Context, id uuid.UUID) error {
	sqlQueryUpdate := `UPDATE contracts SET status = 'expired' WHERE id = $1`
	_, err := cr.db.ExecContext(ctx, sqlQueryUpdate, id)
	return err
}
