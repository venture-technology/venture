...
func NewContractService(contractservice *service.ContractService) *ContractController {
...
func (ct *ContractController) RegisterRoutes(router *gin.Engine) {
	...
	api.GET("/:record/invoice", ct.GetInvoiceFromContract) // para verificar todas as faturas daquele contrato
	api.GET("/:record/invoice/:id", ct.GetInvoice)         // para verificar uma fatura de um contrato especifico
